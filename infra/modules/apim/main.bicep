...
module openAIBackend 'backend.bicep' = if (length(openAIInstances) > 0) {
...
module docIntelBackend 'backend.bicep' = if (length(docIntelInstances) > 0) {
...
//===========================================================
module openAIAPI 'api.bicep' = if (length(openAIInstances) > 0) {
  ...
  params: {
    ...
    // Policy properties
    policyContent: loadTextContent('policies/openai/inbound.xml')
...
module docIntelAPI 'api.bicep' = if (length(docIntelInstances) > 0) {
  ...
  params: {
    ...
    // Policy properties
    policyContent: loadTextContent('policies/docIntel/inbound.xml')
...
output subscriptions object = {
  openAI: length(openAIInstances) > 0 ? openAIAPI.outputs.apiSubscriptionId : ''
  docIntelligence: length(docIntelInstances) > 0 ? docIntelAPI.outputs.apiSubscriptionId : ''
}

output endpoints object = {
  openAI: length(openAIInstances) > 0 ? '${_apim.properties.gatewayUrl}/${openAIAPI.outputs.apiPath}' : ''
  docIntelligence: length(docIntelInstances) > 0 ? '${_apim.properties.gatewayUrl}/${docIntelAPI.outputs.apiPath}' : ''
}
